...
export const merchantAPI = {
  // 商家注册（无需鉴权）
  register: (merchantData) => api.post('/merchants/register', merchantData),
  ...
  // 搜索商家
  searchMerchants: (params = {}) => api.get('/merchants/search', { params }),
  
  // 根据ID获取商家
  getMerchantById: (id) => api.get(`/merchants/${id}`),
  
  // 更新商家基本信息
  updateMerchantInfo: (id, merchantData) => api.put(`/merchants/${id}`, merchantData),
  
  // 删除商家
  deleteMerchant: (id) => api.delete(`/merchants/${id}`),
  
  // 更新商家状态
  updateMerchantStatus: (id, status) => api.patch(`/merchants/${id}/status`, { status }),
  
  // 更新特色菜（餐饮类）
  updateSpecialDishes: (id, dishes) => api.put(`/merchants/${id}/dishes`, { specialDishes: dishes }),
  
  // 更新房型（住宿类）
  updateRoomTypes: (id, rooms) => api.put(`/merchants/${id}/rooms`, { roomTypes: rooms }),
  
  // 更新服务（旅行社/活动提供商/出行服务）
  updateServices: (id, services) => api.put(`/merchants/${id}/services`, { services }),
  
  // 获取商家统计信息
  getMerchantStats: () => api.get('/merchants/stats')
};

// 文件上传API
export const uploadAPI = {
  // 获取SuperBed临时验证参数
  getSuperbedAuth: () => api.get('/upload/superbed/auth'),

  // 上传到SuperBed图床（带降级处理）
  uploadToSuperbed: async (file) => {
    try {
      // 检查文件大小（限制为5MB）
      const maxSize = 5 * 1024 * 1024; // 5MB
      if (file.size > maxSize) {
        throw new Error(`图片文件过大，请选择小于 ${Math.round(maxSize / 1024 / 1024)}MB 的图片`);
      }

      // 1. 获取临时验证参数
      const authResponse = await api.get('/upload/superbed/auth');
      console.log('获取认证参数:', authResponse);
      const { id, ts, sign } = authResponse;
      ...
      formData.append('compress', 'true'); // 启用压缩
      
      console.log('开始上传到SuperBed...', {
        fileName: file.name,
        fileSize: `${(file.size / 1024 / 1024).toFixed(2)}MB`,
        fileType: file.type
      });
      
      // 3. 上传到SuperBed - 使用较短的超时时间
      ...
      console.log('SuperBed响应:', uploadResponse.data);
      
      // 4. 返回统一格式
      if (uploadResponse.data.err === 0) {
        return {
          success: true,
        ...
        throw new Error(uploadResponse.data.msg || 'SuperBed上传失败');
      }
    } catch (error) {
      console.error('上传到SuperBed失败:', error);
      
      // 如果是超时或网络错误，抛出特定错误以便降级处理
      if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
        const timeoutError = new Error('SuperBed上传超时，请检查网络连接或稍后重试');
      ...
      if (error.response) {
        ...
        if (error.response.status >= 500) {
          const serverError = new Error('SuperBed服务暂时不可用，请稍后重试');
  ...
  // 带降级的智能上传（优先SuperBed，失败则本地）
  uploadWithFallback: async (file) => {
    try {
      // 首先尝试SuperBed上传
      return await uploadAPI.uploadToSuperbed(file);
    } catch (error) {
      console.warn('SuperBed上传失败，尝试本地上传:', error.message);
      
      // 如果是超时、服务器错误或网络问题，降级到本地上传
      if (error.code === 'TIMEOUT' || error.code === 'SERVER_ERROR' || 
          error.code === 'ECONNABORTED' || error.message.includes('Network Error')) {
        
        console.log('降级到本地上传...');
        try {
          const localResponse = await uploadAPI.uploadMerchantImage(file);
          return {
            success: true,
          ...
          console.error('本地上传也失败:', localError);
  ...
  // 上传商家图片
  uploadMerchantImage: (file) => {
    const formData = new FormData();
    formData.append('image', file);
    return api.post('/upload/merchant/image', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
  ...
  uploadMerchantImages: (files) => {
    const formData = new FormData();
    files.forEach(file => {
      formData.append('images', file);
    });
    return api.post('/upload/merchant/images', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    });
